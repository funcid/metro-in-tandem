<script lang="ts">
    ...
    const updatePassenger = async () => {
        try {
            ...
            errorMessage = 'Failed to update passenger. Please try again later.';
            console.error(err);
        }
    };
...
<main class="flex flex-col justify-center items-center">
    <p class="font-bold text-[40rem] mb-[20rem]">Просмотр/редактирование пассажира</p>
    <div class="flex justify-center bg-white border border-gray-300 shadow-md w-2/5 rounded-[30rem]">
        ...
        {:else if passenger}
            <div class="flex justify-between p-[40rem] gap-[40rem] w-full">
                <form class="flex flex-col gap-[12rem] w-full" on:submit|preventDefault={updatePassenger}>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <select bind:value={passenger.gender} class="shadow appearance-none border rounded-[12rem] p-[12rem] w-full text-gray-700">
                            <option value="Мужчина">Мужчина</option>
                            <option value="Женщина">Женщина</option>
                        </select>
                    </div>
                    <div class="mb-4">
                        <select id="category" bind:value={passenger.category} class="shadow appearance-none border rounded-[12rem] w-full p-[12rem] text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    ...
                    <div>
                        <label class="block text-gray-700">Контактные номера:</label>
                    ...
                    <div class="flex">
                        <label class="block text-gray-700">Имеет кардиостимулятор</label>
                        <input type="checkbox" id="hasPacemaker" bind:checked={passenger.hasPacemaker} class="ml-[12rem] leading-tight w-[32rem] h-[32rem]" />
                    </div>
                    <button type="button" on:click={deletePassenger} class="bg-[#D4212D] hover:bg-red-700 py-[12rem] px-[26rem] rounded-[12rem] items-center text-white w-full">
                        Удалить пассажира
                    </button>
