<script lang="ts">
    ...
    const updateEmployee = async () => {
        ...
        if (selectedStatus === "vacation") {
            employee!.vacations = [
                ...new Set([...employee!.vacations!, ...updatedDates]),
            ];
        } else if (selectedStatus === "sick-leave") {
            employee!.sickLeaves = [
                ...new Set([...employee!.sickLeaves!, ...updatedDates]),
            ];
        } else if (selectedStatus === "day-off") {
            employee!.daysOff = [
                ...new Set([...employee!.daysOff!, ...updatedDates]),
            ];
        } else if (selectedStatus === "work-day") {
            employee!.vacations = employee!.vacations.filter(
                (date) => !updatedDates.includes(date),
            );
            employee!.sickLeaves = employee!.sickLeaves.filter(
                (date) => !updatedDates.includes(date),
            );
            employee!.daysOff = employee!.daysOff.filter(
                (date) => !updatedDates.includes(date),
            );
        }
...
<main class="flex flex-col justify-center items-center">
    ...
    <div class="flex justify-center gap-[40rem]">
        <div
            class="flex w-full lg:w-[auto] justify-center bg-white border border-gray-300 shadow-md rounded-[30rem]"
        >
            {#if loading}
                ...
                <div class="flex justify-between p-[40rem] gap-[40rem]">
                    ...
                    >
                        ...
                        <div class="flex gap-[30rem] mt-[30rem]">
                            <button
                                type="submit"
                                class="bg-blue-500 min-h-[88rem] hover:bg-blue-700 py-[12rem] px-[26rem] rounded-[12rem] items-center text-white w-full"
                            >
                                Сохранить
                            </button>
                            <button
        ...
        <div
            class="flex lg:w-[auto] justify-center bg-white border border-gray-300 shadow-md rounded-[30rem]"
        >
            {#if loading}
                <p class="text-gray-500">Loading application...</p>
            {:else if errorMessage}
                    ...
                    <div class="flex flex-col justify-between gap-[20rem]">
                        ...
                        <div class="flex gap-[30rem]">
                            ...
                            <select
                                ...
                                class="min-h-[88rem] shadow appearance-none border rounded-[12rem] p-[12rem] w-full text-center text-gray-700"
                            >
